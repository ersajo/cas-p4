<?php
	...
	class mainModel{
		...
		public function limpiarCadena($cadena){
			...
			$palabras=["<script>","</script>","<script src","<script type=","SELECT "," SELECT ",
			"DELETE FROM","INSERT INTO","DROP TABLE","DROP DATABASE","TRUNCATE TABLE","SHOW TABLES",
			"SHOW DATABASES","<?php","?>","--","^","<",">","==",";","::"];
			...
			//Funcion protejida actualizar datos
			...
			protected function actualizarDatos($tabla,$datos,$condicion){
				...
				$sql->bindParam($condicion["condicion_marcador"],$condicion["condicion_marcador"]);

				$sql->execute();

				return $sql;
			}
            //Funcion protegida eliminar
	...
	protected function paginadorTablas($pagina,$numeroPaginas,$url,$botones){
		...
		if ($pagina<=1) {
			...
			$tabla.='
			<a class="pagination-previuos" href="'.$url.($pagina-1).'/"><i class="fas fa-arrow-alt-circle-left"></i> &nbsp; Anterior</a>
			<ul class="pagination-list">
		...
		if ($pagina==$numeroPaginas) {
			...
			<a class="pagination-next is disabled" disabled ><i class="fas fa-arrow-alt-circle-right"></li> &nbsp; Siguiente </a>
			';
		}else{
			$tabla.='
			<li><span class="pagination-ellipsis">&hellip;</span></li>
			...
			<a class="pagination-next" href="'.$url.($pagina+1).'/"><i class="fas fa-arrow-alt-circle-right"></li>&nbsp; Siguiente</a>
			';
		}
		$tabla.='</nav>';
		return $tabla;
